...
for f in glob.glob("A_set/"+args+".json"):
    filename = os.path.basename(f).split(".")[0]
    print (filename)

    if os.path.exists(f"output\{filename}_relaxed.txt"):
        continue
    ...
    penalty_resource_min = model.binary_var_dict([(c, t) for c in data['Resources'].keys() for t in range(1, data['T']+1)], name='penalty_r_min' )
    penalty_resource_max = model.binary_var_dict([(c, t) for c in data['Resources'].keys() for t in range(1, data['T']+1)], name='penalty_r_max' )
    penalty_exclusion = model.binary_var_dict([(itv, itv2, t) for itv in data['Interventions'].keys() for itv2 in data['Interventions'].keys()  for t in range(1, data['T']+1)],name='penalty_exclusion')
    ...
    for itv in data['Interventions'].keys():
        for itv2 in data['Interventions'].keys():
            for t in range(1, data['T']+1):
                penalty += C2 * penalty_exclusion[itv, itv2, t]

    for c in data['Resources'].keys():
        for t in range(1, data['T']+1):
            penalty += C1 * (penalty_resource_max[c, t] + penalty_resource_min[c, t])
    model.minimize(obj_1 + penalty)
    ...
    for t in range(1, data['T']+1):
        for c in data['Resources'].keys():
            ...
            # model.add(model.if_then(resource_c_t >= data['Resources'][c]['max'][t-1] + 0.001, penalty_resource_max[c,t] == 1))
            # model.add(model.if_then(resource_c_t <= data['Resources'][c]['min'][t-1] - 0.001, penalty_resource_min[c,t] == 1))
            try:
    ...
    try:
        with open(f"output\{filename}_relaxed.txt", "w") as f:
            for itv in data['Interventions'].keys():
