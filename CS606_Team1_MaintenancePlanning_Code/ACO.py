...
if __name__ == '__main__':
    ...
    for i in range(1,max_iter):  # 对每次迭代
        ...
        # 顺便记录历史最好情况
        index_best = y.argmin()
        print('after',i,'iterations',', found objective value', round(y[index_best],2),'with',y_penalty[index_best],'violations.')
        ...
        if time.time()- timing_start > time_lst[t]-2:
            ...
            # Storing solutions
            with open("output/"+ args+"_ACO_"+str(time_lst[t])+"s.txt", "w") as f: 
    ...
    with open("output/"+args+"_ACO_report.txt", "w") as f:
